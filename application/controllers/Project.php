...
class Project extends CI_Controller {
	...
	public function getProjectData(){
		...
		$dataSrch = array(
			...
			'bra_id' 		=> $this->input->post('proj_id'),
			'bra_nm' 		=> $this->input->post('projNm'),
			'bra_nm_kh' 	=> $this->input->post('projNmKh'),
			'bra_phone1' 	=> $this->input->post('projPhone'),
			'bra_type_id' 	=> $this->input->post('projType'),
			'srch_all' 		=> $this->input->post('srchAll')
		);
		$data["OUT_REC"] = $this->M_project->selectProject($dataSrch);
		$data["OUT_REC_CNT"] = $this->M_project->countProject($dataSrch);
