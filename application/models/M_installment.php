<?php
	class M_installment extends CI_Model{
        ...
        public function selectProductSold($dataSrch){
            $sql = "select sell_data.sell_id
                        	,pro_sell_price
                        	,pro_data.pro_id
                        	,pro_data.pro_code
        ...
        public function selectMaxInstDate(){
            $sql="SELECT DATE_FORMAT(max(inst_date), '%Y-%m') as inst_date from tbl_installment
                inner join tbl_sell on tbl_sell.sell_id = tbl_installment.sell_id
                where tbl_sell.useYn = 'Y'
                and tbl_installment.useYn = 'Y'";
            return $this->db->query($sql)->result();
        }
